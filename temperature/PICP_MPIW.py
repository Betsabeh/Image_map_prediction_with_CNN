...
def plot_calibration_and_intervals(df):
    def plot_calibration(df):
        ...
        tooltip = [
            alt.Tooltip("y_true:Q", format="d", title="Actual"),
            alt.Tooltip("y_pred:Q", format="d", title="Predicted"),
            alt.Tooltip("y_pred_low:Q", format="d", title="Predicted Lower"),
            alt.Tooltip("y_pred_upp:Q", format="d", title="Predicted Upper"),
        ]
...
#------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
def calculate_prediction_interval(observed_values, predicted_values, confidence_level):
    prediction_errors = observed_values - predicted_values
    sorted_errors = np.sort(prediction_errors)
    lower_quantile = (1 - confidence_level) / 2
    upper_quantile = 1 - lower_quantile
    lower_index = int(lower_quantile * len(sorted_errors))
    upper_index = int(upper_quantile * len(sorted_errors))
    lower_bound = predicted_values + sorted_errors[lower_index]
    upper_bound = predicted_values + sorted_errors[upper_index]
    return lower_bound, upper_bound

def calculate_picp(observed_values, lower_bound, upper_bound):
    within_interval = (observed_values >= lower_bound) & (observed_values <= upper_bound)
    picp = np.mean(within_interval)
    return picp

def PICP_MPIW(X,Y,model_name):
    ...
    lower_bound, upper_bound = calculate_prediction_interval(Y_Test, predicted_values, confidence_level)
    picp = calculate_picp(Y_Test, lower_bound, upper_bound)
    print("PICP:", picp)
    MPIW = calculate_mpiw(Y_Test, predicted_values, confidence_level)
    print("MPIW:",MPIW)

#-------------------------------------------------------------------------------
def calculate_prediction_interval_width(observed_values, predicted_values, confidence_level):
    prediction_errors = observed_values - predicted_values
    sorted_errors = np.sort(prediction_errors)
    lower_quantile = (1 - confidence_level) / 2
    upper_quantile = 1 - lower_quantile
    ...
    lower_bound = predicted_values + sorted_errors[lower_index]
    upper_bound = predicted_values + sorted_errors[upper_index]
    interval_widths = np.abs(upper_bound - lower_bound)
    return interval_widths

def calculate_mpiw(observed_values, predicted_values, confidence_level):
    interval_widths = calculate_prediction_interval_width(observed_values, predicted_values, confidence_level)
