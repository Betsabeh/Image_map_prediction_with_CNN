...
def uncertainty(X,Y,model_name):
    #To estimate uncertainty using Quantile Regression Forest
    ...
    # Predict the median (quantile=0.5)
    y_pred = model.predict(X)

    # Predict the quantiles of interest
    quantile_preds = []
    for quantile in quantiles:
        quantile_pred = model.predict(X)
        quantile_preds.append(quantile_pred)

    # Calculate the uncertainty as the range between the upper and lower quantiles
    uncertainty = np.abs(np.array(quantile_preds) - np.tile(y_pred, (len(quantiles), 1)))

    # Print the uncertainty for each quantile
    for i, quantile in enumerate(quantiles):
        print(f'Quantile {quantile}: {uncertainty[i]}')
